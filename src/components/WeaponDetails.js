...
const WeaponDetails = () => {
  ...
  const loadWeaponImage = (weaponName) => {
    ...
    for (const format of imageFormats) {
      const potentialPath = `${window.location.origin}/images/weapons/${weaponName.replace(/[^a-zA-Z0-9]/g, "")}.${format}`;
      const img = new Image();
      img.onload = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3001/api/details-weapon/${name}`) // Adjust the API endpoint accordingly
      ...
      .then(data => {
        ...
        // Load the image dynamically when weapon data is fetched
        loadWeaponImage(data.name);
      })
      .catch(error => {
